...
namespace WebsiteDatVeXeKhach08.Controllers
{
    public class CustomerController : Controller
    {
        ...
        public ActionResult SearchBusByDate(DateTime? searchDate)
        {
            ...
            // Truy vấn csdl để lấy danh sách các chuyến xe còn trống (AvailableSeats>0) theo ngày
            var availableBuses = db.Buses
                .Where(b => DbFunctions.TruncateTime(b.DepartureTime) == DbFunctions.TruncateTime(date)
                && b.AvailableSeats > 0)
        ...
        public ActionResult SearchRouteByDate(DateTime? searchDate)
        {
            ...
            // Truy vấn csdl để lấy danh sách các tuyến đường có xe bus còn trống theo ngày
            var availableRoutes = db.Routes
                .Where(route => db.Buses.Any(bus => bus.RouteID == route.RouteID 
                && DbFunctions.TruncateTime(bus.DepartureTime) == DbFunctions.TruncateTime(date) 
